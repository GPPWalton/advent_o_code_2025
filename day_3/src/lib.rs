pub mod day_3_solution {
    ...
    fn search_bank(battery_row: &Vec<char>) -> SearchResult
    {
        let mut index = 0;
        let mut result = '0';
        ...
        SearchResult{value: result, index}
    }
    fn insertion_sort (result: Vec<SearchResult>) -> Vec<SearchResult>{

        let mut sorted_results = result;
        for i in 0..sorted_results.len(){
            for j in (0..i).rev() {
                if sorted_results[j].index > sorted_results[j+1].index{
                    sorted_results.swap(j+1, j);
                }
                else {
                    break
                }
        ...
        if let Ok(lines) = read_lines("./day_3/src/input.txt") {
            ...
            for line in lines.map_while(Result::ok) {
                ...
                if !p2_flag {
                    ...
                    // let parsed_joltage: usize = find_max_joltage_p2(bank).parse().unwrap();
                    // max_jolt += parsed_joltage;
                }
            }
            max_jolt
